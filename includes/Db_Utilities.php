...
abstract class Db_Utilities
{
    ...
    public function get( $row_id )
    {
        ...
        return $wpdb->get_row(
            $wpdb->prepare(
                "SELECT * FROM $this->table_name WHERE $this->primary_key = %s LIMIT 1;", $row_id
            )
        );
    }
    ...
    /**
     * Retrieve a row by a two specified column / value
     *
     * @access public
     * @since  2.1
     * @return object
     */
    public function get_row_by_invoice( $invoice_number, $reqeust_reference, $form=false )
    {
        global $wpdb;
        $invoice_number = esc_sql($invoice_number);
        $reqeust_reference = esc_sql($reqeust_reference);
        $column_invoice = !$form ? "invoice_number" : "invoice_number_filled_against";
        $column_ref = !$form ? "request_reference" : "request_ref_filled_against";
        return $wpdb->get_row(
            $wpdb->prepare(
