 ...
 * @since      1.0.0
 */
class BusinessPremiseRenewal extends RegistrationUtilities
{
     ...
     * @var boolean
     */
    protected $reg_form_type = "biz_prem_renewal";
    ...
    public function __construct()
    {
        if (isset($_REQUEST["is_new"]) && $_REQUEST["is_new"]==1) {
            $this->all_input_names = array('company_name');
        } else {
    ...
    public function check_if_renewal_invoice_is_used()
    {
        global $mtii_biz_prem_db_main;
        $task_performer = new TasksPerformer;
        $renewal_invoice = $this->get_invoice_info_from_db();
        $linked_ngo = $mtii_biz_prem_db_main->get_by('invoice_number_filled_against', $renewal_invoice->invoice_number);
        $renewal_invoice_as_post = get_page_by_title($renewal_invoice->invoice_number, OBJECT, 'mtii_cbs_invoice');
        $renewal_connected_org = get_post_meta($renewal_invoice_as_post->ID, 'connected_org', true);
        $renewal_start_use = get_post_meta($renewal_invoice_as_post->ID, 'start_use', true);

        if ($renewal_connected_org || $renewal_start_use || $linked_ngo) {
            $encoded_payment_catg = urlencode(openssl_encrypt("business-premise", "AES-128-ECB", "X340&2&230rTHJ34"));
            $error_title = 'Used Renewal Invoice';
            $error_body = 'This Invoice has already being used for ';
            $error_body .= ' Renewal. You should raise a new Invoice and make payment';
    ...
    protected function update_renewal_invoice_db_table($invoice_as_post, $connected_name, $linked_organization=null, $invoice_in_db=null, $new_biz_prem=true)
    {
        global $mtii_biz_prem_db_main;
        ...
        if (!$new_biz_prem && $invoice_in_db) {
            ...
            $mtii_biz_prem_db_main->update($main_form_id, $linked_ngo_array);
        }
    }
    ...
    protected function add_all_info_to_db()
    {
        ...
        if (isset($_REQUEST["is_new"]) && $_REQUEST["is_new"]==1) {
            $this->_register_new_online_biz_prem();
        } else {
            $linked_ngo = $mtii_biz_prem_db_main->get_by('invoice_number_filled_against', $form_values["expired_invoice_number"]);
            ...
            if (!$linked_ngo || $linked_ngo=='') {
                ...
                $error_body .= 'If you are a new Business Premise, please register first as a new organization.';
                ...
                if ($expired_invoice_in_db && $expired_invoice_in_db!=null) {
                    ...
                    if ($days_since_last_payment<366) {
                        ...
                        $body = 'It seems the previous registration for '.strtoupper($linked_ngo->name_of_company);
                        ...
                        $linked_name_from_db = $linked_ngo->name_of_company;
                        ...
                        if ($update_inv_table && (!$error_one || $error_one=='')) {
                            $this->update_renewal_invoice_db_table(
                        ...
                        if (!$error_one && !$error_two) {
                            $this->delete_post_from_title($form_values["expired_invoice_number"], 'mtii_biz_prem_reg');
                            $this->create_registration_custom_post($linked_name_from_db, 'mtii_biz_prem_reg');
                            $title = 'Renewal successful';
                            $body = 'Your Premise renewal Renewal for '.strtoupper($linked_ngo->name_of_company);
                            $body .= ' was successful. Please wait for new approval by Admin. Thank you!';
                            ...
                            $title = 'Renewal Not Successful';
                            $body = 'Your business premise renewal for '.strtoupper($linked_ngo->name_of_company);
                            $body .= ' was not successful. Please contact Admin! Thank you!';
                            echo $task_performer->output_inline_notification($title, $body, 'is-error');
                        } else {
                            $title = 'Renewal successful with errors';
                            $body = 'Your premise renewal for '.strtoupper($linked_ngo->name_of_company);
    ...
    private function _register_new_online_biz_prem()
    {
        global $mtii_biz_prem_db_main;
        global $wpdb;
        $task_performer = new TasksPerformer;
        $all_registered_biz_prem = json_decode($task_performer->get_all_registered_business_premise());
        $form_values = $this->form_values;
        if (!$task_performer->in_arrayi($form_values["company_name"], $all_registered_biz_prem)) {
            $title = "Organization name not Found!";
            $body = "Sorry! There is presently no offline registration ";
            $body .= "with this name. Please make payment as a fresh organization if you have not registered";
            $body .= " offline. If you have however registered offline, please contact Admin. Thank you. <br /><br />";
            echo $task_performer->output_inline_notification($title, $body, 'is-error');
        } else {
            ...
            $linked_ngo = $mtii_biz_prem_db_main->get_by('invoice_number_filled_against', $invoice_info->invoice_number);
            $linked_ngo_from_name = $mtii_biz_prem_db_main->get_by('name_of_company', $form_values["company_name"]);
            if ($linked_ngo_from_name) {
                ...
                $form_values["name_of_company"] = strtoupper($form_values["company_name"]);
                ...
                if (!$wpdb->last_error) {
                    $this->update_renewal_invoice_db_table($invoice_info_as_post, strtoupper($form_values["company_name"]));
                    $title = "Renewal Successful!";
                    $body = "Your renewal was successful. Click the button below to fill in other details <br /><br />";
                    $body .= '<a class="round-btn-mtii small-btn" href="';
    ...
    public function mtii_save_reg_form_info()
    {
        if (isset($_POST["mtii_form_submit"]) && isset($_POST['main_registration_nonce'])
            ...
            if ($this->check_if_renewal_invoice_is_used()) {
                unset($_POST);
                return;
            } else {
